...
{(item.member && "avg_rating" in item.member && (item.member as { avg_rating: number; review_count: number }).avg_rating > 0) && (
                      <>
                        <span className="flex items-center gap-1 text-sm text-gray-700">
                          ...
                          {(item.member as { avg_rating: number; review_count: number }).avg_rating.toFixed(1)} ({(item.member as { avg_rating: number; review_count: number }).review_count} รีวิว)
                        </span>
                      </>
                    )}
